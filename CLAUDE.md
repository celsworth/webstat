...
**Top erroring URLs** (`top_error_urls`, gated by `enable_top_error_urls`): a per-period **long** table — one row per `(period, url_id, status)` with `hits` and `bandwidth`. *Every* 4xx/5xx code is recorded individually (no fixed columns, no storage-time catch-alls). Populated in `aggregate_entry` for any `status >= 400` (respecting `HideMask::TOP_URLS`). URLs are interned through a shared `urls (id, url)` table; the long format emits one row per status code per URL, so interning avoids repeating the URL string. Bounding operates at **URL granularity** (totals summed across status codes): `pretrim_error_urls` at month-end keeps the union of top-N URLs by total hits and total bandwidth; `cull_period` removes URLs whose per-URL totals fall below 1/10 of the N-th-best; `finalize_month` trims to the union of top-N by `SUM(hits)` and top-N by `SUM(bandwidth)`, then prunes orphaned `urls` rows (rollback prunes them too). Reports fold the stored per-code data into columns at **report time**, driven by config `error_url_codes` (default `[400,401,403,404,422,429,500,502,503]`): one column per listed code, with unlisted 4xx folded into a "4xx" column and unlisted 5xx into "5xx" (each omitted if empty). Changing `error_url_codes` needs no reprocessing. Rendered as a sortable panel on the month, year, and overview pages.
